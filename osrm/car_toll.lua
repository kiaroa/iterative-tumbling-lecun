-- Fork of Project-OSRM's stock profiles/car.lua (v26.8.0), for the toll-minimising route
-- service (Phase 1a). This file is a byte-for-byte copy of the upstream profile body below
-- this header comment.
--
-- Investigation finding (2026-08-10): the spec calls for adding `toll` to `excludable` and
-- tagging `toll=yes` ways to that class. As of osrm-backend v26.8.0, upstream profiles/car.lua
-- already does both:
--   - `classes` includes 'toll' and `excludable` includes Set {'toll'}.
--   - lib/way_handlers.lua's handle_classification sets forward/backward_classes.toll = true
--     whenever the way carries toll=yes (forward) / a matching reverse tag (backward), guarded
--     by `allowed_classes["toll"]` which is populated from the profile's `classes` sequence.
--   - `avoid` deliberately leaves toll commented out, so tolls are included in the default
--     route and only excluded when a request passes `exclude=toll`.
-- No functional changes were required to satisfy the deliverable; this file is kept as a named
-- fork (rather than referencing /opt/car.lua directly) so future phases have a stable, editable
-- copy if a real customisation (e.g. a different toll speed penalty) is ever needed.
